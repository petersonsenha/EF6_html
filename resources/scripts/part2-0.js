//Mostrar/Ocultar Sugestão de filme
//part2-0-brincadeiras.html
if (typeof mudarVisibilidade !== "undefined") {
  document.getElementById("btnsugestao-19").onclick = () =>
    mudarVisibilidade(
      document.getElementById("bloco-sugestao-19"),
      document.getElementById("instrucao-19"),
      document.getElementById("sugestao-19")
    );

  document.getElementById("sugestao-20").onclick = () =>
    mudarVisibilidade(
      document.getElementById("bloco-sugestao-20"),
      document.getElementById("instrucao-20"),
      document.getElementById("sugestao-20")
    );
  document.getElementById("sugestao-21").onclick = () =>
    mudarVisibilidade(
      document.getElementById("bloco-sugestao-21"),
      document.getElementById("instrucao-21"),
      document.getElementById("sugestao-21")
    );

  document.getElementById("sugestao-22").onclick = () =>
    mudarVisibilidade(
      document.getElementById("bloco-sugestao-22"),
      document.getElementById("instrucao-22"),
      document.getElementById("sugestao-22")
    );
}
